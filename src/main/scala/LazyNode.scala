...
object LazyNode {
   ...
   * The first positions are (row: 1, col: 1), then the left and right jump the row and the right jumps the column
  ...
  def getInfinitePositions(row: Int, col: Int): LazyNode[(Int, Int)] =
    LazyLeaf((row, col), () => getInfinitePositions(row + 1, col), () => getInfinitePositions(row + 1, col + 1))
